...
exports.verifyEmail = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.redirect("http://:5173/verify-email?status=error");
    ...
    res.redirect("http://:5173/verify-email?status=success");
  } catch (err) {
    console.error(err.message);
    res.redirect("http://:5173/verify-email?status=error");
  }
};
...
exports.forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const resetUrl = `http://:5173/reset-password?token=${resetToken}&email=${email}`;
