                ...
                <div className="Cards-container">
                    <section className="courses-grid">
                        ...
                        {!loading && !error && courses.map((c) => (
                            <CourseCard key={c._id} src={c.image || `https://placehold.co/600x400/ddd/sss?text=${encodeURIComponent(c.name)}`} title={c.name} courseCode={c.courseCode} tasks={c.taskCount || 0} createdCourse={c.code || 0} onView={() => handleView(c)} onClick={() => handleLeave(c.code)} />
                        ))}
                    </section>
                </div>
